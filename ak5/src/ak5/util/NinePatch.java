...
public class NinePatch implements Drawable {
	...
	Texture[] patches = new Texture[9];

	Texture rawImage;
	/* Dimensions except middle patch. */
	int minWidth, minHeight;

	public NinePatch(Texture image, int left, int right, int top, int bottom) {
		// this(image);
		patches[TOP_LEFT] = image.region(0, 0, left, top);
	}

	//	public NinePatch(Texture image) {
	...
	//	}
	//
	//	Line parseInterval(Texture image, IntervalType intervalType) {
