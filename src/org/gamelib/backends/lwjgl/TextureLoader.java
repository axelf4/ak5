...
public class TextureLoader {
	...
	private HashMap<URL, Image> table = new HashMap<URL, Image>();
	 ...
	 */
	public Image getTexture(URL url) throws IOException {
		Image tex = table.get(url);

		if (tex != null) {
			return tex;
		}

		tex = getTexture(url, GL_TEXTURE_2D, // target
	 ...
	 * @throws IOException Indicates a failure to access the resource
	 */
	public Image getTexture(URL url, int target, int dstPixelFormat, int minFilter, int magFilter)
			throws IOException {
		int srcPixelFormat;

		// create the texture ID for this texture
		int textureID = createTextureID();
		Image texture = new LWJGLImage(target, textureID);
	...
	private BufferedImage loadImage(URL url)throws IOException {
		BufferedImage img = ImageIO.read(new FileInputStream("image.png"));
		return img;
	}
}