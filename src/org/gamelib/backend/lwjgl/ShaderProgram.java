...
public class ShaderProgram {
	...
	/**
	 * Creates a new shader program with the given vertex and fragment shader source code. The given source code is compiled, then the shaders attached and linked. If shaders are not supported on this system (isSupported returns false), a LWJGLException will be thrown. If one of the shaders does not compile successfully, a LWJGLException will be thrown. If there was a problem in linking the shaders to the program, a LWJGLException will be thrown and the program will be deleted. Before linking, the specified attribLocations will be bound using glBindAttribLocation.
	...
	public ShaderProgram(String vertexShaderSource, String fragShaderSource)
		...
		if (vertexShaderSource == null || fragShaderSource == null) throw new IllegalArgumentException("shader source must be non-null");
		if (!isSupported()) throw new LWJGLException("no shader support found; shaders require OpenGL 2.0");
	...
	protected int createProgram() throws LWJGLException {
		...
		if (program == 0) throw new LWJGLException("could not create program; check ShaderProgram.isSupported()");
		return program;
	}

	/**
	 * Compiles a shader from source and returns its handle. If the compilation failed, a SlimException will be thrown. If the compilation had error, info or warnings messages, they will be appended to this program's log.
	...
	protected int compileShader(int type, String source) throws LWJGLException {
		...
		if (shader == 0) throw new LWJGLException("could not create shader object; check ShaderProgram.isSupported()");
		...
		if (err != null && err.length() != 0) log += shaderTypeString(type) + " compile log:\n" + err + "\n";
		if (status == GL11.GL_FALSE) throw new LWJGLException(log.length() != 0 ? log : "Could not compile " + shaderTypeString(type));
		return shader;
	}
	 ...
	 * @throws SlimException if this program is invalid (released) or if the link was unsuccessful
	 */
	protected void linkProgram(List<VertexAttrib> attribLocations)
			throws LWJGLException {
		if (!valid()) throw new LWJGLException("trying to link an invalid (i.e. released) program");
		...
		if (comp == GL11.GL_FALSE) throw new LWJGLException(log.length() != 0 ? log : "Could not link program");
		fetchUniforms();
		fetchAttributes();
	}
	...
	/**
	 * Enables this shader for use -- only one shader can be bound at a time. Calling bind() when another program is bound will simply make this object the active program.
	 * @throw IllegalStateException if this program is invalid
	 */
	public void bind() {
		if (!valid()) throw new IllegalStateException("trying to enable a program that is not valid");
	...
	/**
	 * If this program has not yet been released, this will releases this program and its shaders. To only release the shaders (not the program itself), call disposeShaders(). Programs will be considered "invalid" after being released, and should no longer be used.
	 */
	public void dispose() {
		if (program != 0) {
			disposeShaders();
	...
	/**
	 * A shader program is "valid" if it's ID is not zero. Upon releasing a program, the ID will be set to zero.
	...
	/**
	 * Returns the location of the uniform by name. If the uniform is not found and we are in strict mode, an IllegalArgumentException will be thrown, otherwise -1 will be returned if no active uniform by that name exists.
	...
	public int getUniformLocation(String name) {
		...
		} else location = locI.intValue();
		if (location == -1) throw new IllegalArgumentException("no active uniform by name '" + name + "'"); // throw an exception if not found...
		return location;
	}

	Attrib attrib(String name) {
		for (int i = 0; i < attributes.length; i++) {
			if (name.equals(attributes[i].name)) return attributes[i];
		}
		throw new IllegalArgumentException("no active attribute by name '" + name + "'"); // throw an exception if not found...
	}

	/**
	 * Returns the location of the attribute by name. If the attribute is not found and we are in strict mode, an IllegalArgumentException will be thrown, otherwise -1 will be returned if no active attribute by that name exists.
	...
	/**
	 * Returns the type of the attribute by name. If the attribute is not found and we are in strict mode, an IllegalArgumentException will be thrown, otherwise -1 will be returned if no active attribute by that name exists.
	 * @param name the attribute name
	 * @return the ID (location) in the shader program
	 */
	...
	/**
	 * Returns the size of the attribute by name (i.e. for arrays). If the attribute is not found and we are in strict mode, an IllegalArgumentException will be thrown, otherwise -1 will be returned if no active attribute by that name exists.
	 * @param name the attribute name
	 * @return the ID (location) in the shader program
	 */
	...
	/**
	 * Creates and returns an array for all active attributes that were found when linking the program.
	...
	/**
	 * Creates and returns an array for all active uniforms that were found when linking the program.
	 * @return an array list of active uniform names
	 */
	...
	/**
	 * Returns true if an active attribute by the given name was found when linking.
	 * @param name the active attribute name
	 * @return true if the attribute was found
	 */
	public boolean hasAttribute(String name) {
		for (int i = 0; i < attributes.length; i++)
			if (name.equals(attributes[i].name)) return true;
		return false;
	}
	...
	private FloatBuffer uniformf(int loc) {
		if (fbuf16 == null) fbuf16 = BufferUtils.createFloatBuffer(16);
		fbuf16.clear();
		if (loc == -1) return fbuf16;
		getUniform(loc, fbuf16);
		return fbuf16;
	}

	private IntBuffer uniformi(int loc) {
		if (ibuf4 == null) ibuf4 = BufferUtils.createIntBuffer(4);
		ibuf4.clear();
		if (loc == -1) return ibuf4;
	...
	/**
	 * Retrieves data from a uniform and places it in the given buffer. If strict mode is enabled, this will throw an IllegalArgumentException if the given uniform is not 'active' -- i.e. if GLSL determined that the shader isn't using it. If strict mode is disabled, this method will return <tt>true</tt> if the uniform was found, and <tt>false</tt> otherwise.
	...
	/**
	 * A convenience method to retrieve an integer/sampler2D uniform. The return values are undefined if the uniform is not found.
	...
	/**
	 * A convenience method to retrieve an ivec2 uniform; for maximum performance and memory efficiency you should use getUniform(int, IntBuffer) with a shared buffer.
	 * @param loc the uniform location
	...
	/**
	 * A convenience method to retrieve an ivec2 uniform; for maximum performance and memory efficiency you should use getUniform(int, IntBuffer) with a shared buffer. The return values are undefined if the uniform is not found.
	 * @param name the uniform name
	 * @return a newly created int[] array with 2 elements; e.g. (x, y)
	 */
	...
	/**
	 * A convenience method to retrieve an ivec3 uniform; for maximum performance and memory efficiency you should use getUniform(String, IntBuffer) with a shared buffer.
	...
	/**
	 * A convenience method to retrieve an ivec3 uniform; for maximum performance and memory efficiency you should use getUniform(String, IntBuffer) with a shared buffer. The return values are undefined if the uniform is not found.
	 * @param name the name of the uniform
	 * @return a newly created int[] array with 3 elements; e.g. (x, y, z)
	 */
	...
	/**
	 * A convenience method to retrieve an ivec4 uniform; for maximum performance and memory efficiency you should use getUniform(String, IntBuffer) with a shared buffer.
	 * @param loc the location of the uniform
	...
	/**
	 * A convenience method to retrieve an ivec4 uniform; for maximum performance and memory efficiency you should use getUniform(String, IntBuffer) with a shared buffer. The return values are undefined if the uniform is not found.
	 * @param name the name of the uniform
	 * @return a newly created int[] array with 2 elements; e.g. (r, g, b, a)
	 */
	...
	/**
	 * A convenience method to retrieve a float uniform. The return values are undefined if the uniform is not found.
	 * @param name the uniform name
	 * @return the value
	 */
	...
	/**
	 * A convenience method to retrieve a vec2 uniform; for maximum performance and memory efficiency you should use getUniform(String, FloatBuffer) with a shared buffer.
	 * @param location the location of the uniform
	...
	/**
	 * A convenience method to retrieve a vec2 uniform; for maximum performance and memory efficiency you should use getUniform(String, FloatBuffer) with a shared buffer. The return values are undefined if the uniform is not found.
	 * @param name the name of the uniform
	 * @return a newly created float[] array with 2 elements; e.g. (x, y)
	 */
	...
	/**
	 * A convenience method to retrieve a vec3 uniform; for maximum performance and memory efficiency you should use getUniform(String, FloatBuffer) with a shared buffer.
	 * @param location the location of the uniform
	...
	/**
	 * A convenience method to retrieve a vec3 uniform; for maximum performance and memory efficiency you should use getUniform(String, FloatBuffer) with a shared buffer. The return values are undefined if the uniform is not found.
	 * @param name the name of the uniform
	 * @return a newly created float[] array with 3 elements; e.g. (x, y, z)
	 */
	...
	/**
	 * A convenience method to retrieve a vec4 uniform; for maximum performance and memory efficiency you should use getUniform(String, FloatBuffer) with a shared buffer.
	 * @param location the location of the uniform
	...
	/**
	 * A convenience method to retrieve a vec4 uniform; for maximum performance and memory efficiency you should use getUniform(String, FloatBuffer) with a shared buffer. The return values are undefined if the uniform is not found.
	 * @param name the name of the uniform
	 * @return a newly created float[] array with 4 elements; e.g. (r, g, b, a)
	 */
	...
	/**
	 * Returns true if the extensions GL_ARB_shader_objects, GL_ARB_vertex_shader, and GL_ARB_fragment shader are present.
